...
# 主要思想
算法主要基于加权平均的方法，用陀螺仪数据补偿加速度计数据。  
Rgyro(n)相当于从陀螺仪数据反解出加速度计数据。  
算法过程如下：  
输入：
> * Racc(n) = [RxAcc(n),RyAcc(n),RzAcc(n)]  
> * RateAxz(n), RateAyz(n)
> * Rest(n-1) = [RxEst(n-1),RyEst(n-1),RzEst(n-1)] 

计算：
> * Axz(n-1), Ayz(n-1)
> * Axz(n), Ayz(n)
> * Rgyro(n) = [RxGyro(n),RyGyro(n),RzGyro(n)]

> * Rest(n) = (Racc(n) + Rgyro(n) * wGyro ) / (1 + wGyro) 

输出：
> * Rest(n) = [RxEst(n),RyEst(n),RzEst(n)] 
...
# 注意
根据实际数据统一单位：degree or rad ; ms or s ; g or 9.8m/s^2
